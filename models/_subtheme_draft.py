...
class UserModel(Module):
    def __init__(self, num_c1, num_c2, num_c3, num_d, dim_v):
        ...
        self.linear_2 = Sequential(
            Linear(self.dim_v * 5, self.dim_v),
            ReLU(),
            Dropout(),
            Linear(self.dim_v, 2),
            Dropout(),
    ...
    def forward(
        self, c1_seq, c2_seq, c3_seq, d_seq, r_seq,
        h_0=None, C1_0=None, C2_0=None, C3_0=None
    ):
        '''
            Args:
                c1_seq: [batch_size, seq_len]
                c2_seq: [batch_size, seq_len]
                c3_seq: [batch_size, seq_len, num_c3_tag]
        ...
        # c3_one_hot_seq: [batch_size, seq_len, num_c3_tag, num_c3]
        c1_one_hot_seq = one_hot(c1_seq, self.num_c1).float()
        c2_one_hot_seq = one_hot(c2_seq, self.num_c2).float()
        c3_one_hot_seq = one_hot(c2_seq, self.num_c3).float()

        # c3_multi_hot_seq: [batch_size, seq_len, num_c3]
        c3_multi_hot_seq = c3_one_hot_seq.sum(-2)

        for c1_one_hot, c2_one_hot, c3_one_hot, c3_multi_hot, v_d, v_r in zip(
            c1_one_hot_seq.permute(1, 0, 2),
            c2_one_hot_seq.permute(1, 0, 2),
            c3_one_hot_seq.permute(1, 0, 2, 3),
            c3_multi_hot_seq.permute(1, 0, 2),
            v_d_seq.permute(1, 0, 2),
            v_r_seq.permute(1, 0, 2)
        ):
            # c1_one_hot: [batch_size, num_c1]
            # c2_one_hot: [batch_size, num_c2]
            # c3_one_hot: [batch_size, num_c3_tag, num_c3]
            # c3_multi_hot: [batch_size, num_c3]
            # v_d, v_r: [batch_size, dim_v]

            # beta1_tilde: [batch_size, 1, 1]
            # beta2_tilde: [batch_size, 1, 1]
            # beta3_tilde: [batch_size, num_c3_tag, 1]
            beta1_tilde = torch.bmm(c1_one_hot.unsqueeze(1), C1)
            beta2_tilde = torch.bmm(c2_one_hot.unsqueeze(1), C2)
            beta3_tilde = torch.bmm(c3_one_hot, C3)

            # beta3_tilde_bar: [batch_size, 1, 1]
            beta3_tilde_bar = beta3_tilde.mean(1, keepdim=True)

            # v_c1: [batch_size, dim_v]
            # v_c2: [batch_size, dim_v]
            # v_c3: [batch_size, num_c3_tag, dim_v]
            v_c1 = (beta1_tilde * self.v_c1).squeeze()
            v_c2 = (beta2_tilde * self.v_c2).squeeze()
            v_c3 = (beta3_tilde * self.v_c3).squeeze()

            v_c1 = torch.reshape(v_c1, [batch_size, self.dim_v])
            v_c2 = torch.reshape(v_c2, [batch_size, self.dim_v])
            v_c3 = torch.reshape(v_c3, [batch_size, num_c3_tag, self.dim_v])

            # v_c3_bar: [batch_size, dim_v]
            v_c3_bar = (beta3_tilde_bar * self.v_c3).squeeze()

            # new_c: [batch_size, 2]
            # new_c1, new_c2: [batch_size, 1]
            # new_c3: [batch_size, num_c3_tag, 1]
            new_c = self.linear_2(
                torch.cat([v_c1, v_c2, v_c3_bar, v_d, v_r], dim=-1)
            )
            new_c1 = new_c[:, :1]
            new_c2 = new_c[:, 1:]
            ...
            new_c3 = self.linear_3(
                torch.cat([
                    v_c1.repeat(1, num_c3_tag, 1),
                    v_c2.repeat(1, num_c3_tag, 1),
                    v_c3,
                    v_d.repeat(1, num_c3_tag, 1),
                    v_r.repeat(1, num_c3_tag, 1),
                ])
            )
            ...
            C3 = C3 * (1 - c3_multi_hot.unsqueeze(-1)) + \
                (new_c3 * c3_one_hot).sum(1).unsqueeze(-1)
        ...
        return alpha_seq, h_seq, C1_seq, C2_seq, C3_seq
