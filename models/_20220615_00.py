...
class UserModel(Module):
    ...
    def forward(
        ...
        for c1_one_hot, c2_one_hot, v_d, v_r in zip(
            ...
            if batch_size == 1:
                v_c1 = v_c1.unsqueeze(0)
                v_c2 = v_c2.unsqueeze(0)
    ...
    def train_model(
        ...
        for i in range(1, num_epochs + 1):
            ...
            for data in train_loader:
                ...
                assert self.window_size <= seq_len

                # Shuffling
                max_indices = np.argmin(m_seq.detach().cpu().numpy(), axis=-1)
                max_indices[max_indices == 0] = seq_len
                max_indices = max_indices - 1

                window_begin = (
                    np.random.uniform(0, 1, size=[batch_size]) * max_indices
                ).astype(int)

                random_perm = np.array(
                    [
                        np.random.permutation(self.window_size)
                        for _ in range(batch_size)
                    ]
                )
                window_random_perm = (window_begin + random_perm.T).T
                window_random_perm = np.minimum(
                    window_random_perm.T, max_indices
                ).T

                indices = np.array(
                    [np.arange(self.window_size) for _ in range(batch_size)]
                )
                window_indices = (window_begin + indices.T).T
                window_indices = np.minimum(window_indices.T, max_indices).T

                window_indices_ = (
                    np.array(
                        [np.arange(batch_size)] * self.window_size
                    ).T.flatten(),
                    window_indices.flatten()
                )
                random_perm_ = (
                    np.array(
                        [np.arange(batch_size)] * self.window_size
                    ).T.flatten(),
                    window_random_perm.flatten()
                )

                c1_seq[window_indices_] = c1_seq[random_perm_]
                c2_seq[window_indices_] = c2_seq[random_perm_]
                d_seq[window_indices_] = d_seq[random_perm_]
                r_seq[window_indices_] = r_seq[random_perm_]
                c1shft_seq[window_indices_] = c1shft_seq[random_perm_]
                c2shft_seq[window_indices_] = c2shft_seq[random_perm_]
                dshft_seq[window_indices_] = dshft_seq[random_perm_]
                rshft_seq[window_indices_] = rshft_seq[random_perm_]
